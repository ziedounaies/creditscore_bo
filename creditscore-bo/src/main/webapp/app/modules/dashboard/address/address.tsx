...
import { getEntities, deleteEntity } from './address.reducer';
...
const Address = () => {
  ...
  const handleMenuClick: any = (e: any, id: string) => {
    if (e?.key === "edit") {
      navigate(`/dashboard/address/${id}/edit`);
  ...
  const handleOkButtonClick = () => {
    ...
    dispatch(deleteEntity(deleteAddressId)); // Use the ID from state for deletion
    setTimeout(() => {
      setVisible(false);
  ...
  const confirmDelete = (addressId: string) => {
    setModalText(`Êtes-vous certain de vouloir supprimer l'adresse ${addressId} ?`);
    setVisible(true);
    setDeleteAddressId(addressId); // Set the ID to delete in state
  };
  ...
  return (
    <div>
      <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between' ,}}>
        <Col>
        <Title  style={{ marginBottom: 0 }} id="address-heading" data-cy="AddressHeading" level={2}>Addresses</Title>
        <Text style={{ marginBottom: 10 }} type="secondary">Adresses des Banques - Localisez les Succursales</Text>
        </Col>
        <div style={{marginTop:15}} className="d-flex justify-content-end">
          ...
          <Button type="primary" onClick={() => navigate('/dashboard/address/new')}>Créer une nouvelle adresse</Button>
        </div>
      </div>
      <Table
