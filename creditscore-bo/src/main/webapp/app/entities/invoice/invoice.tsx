    ...
    <div>
      ...
      <div className="table-responsive">
        {invoiceList && invoiceList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                ...
                <th>
                  Credit Rapport <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {invoiceList.map((invoice, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{invoice.memberUser ? <Link to={`/member-user/${invoice.memberUser.id}`}>{invoice.memberUser.id}</Link> : ''}</td>
                  <td>
                    {invoice.creditRapport ? (
                      <Link to={`/credit-rapport/${invoice.creditRapport.id}`}>{invoice.creditRapport.id}</Link>
                    ) : (
                      ''
                    )}
                  </td>
          ...
          !loading && <div className="alert alert-warning">Aucun Invoice trouvé</div>
        )}
      </div>
      {totalItems ? (
