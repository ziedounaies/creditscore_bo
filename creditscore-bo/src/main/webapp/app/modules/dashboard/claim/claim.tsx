...
const Claim = () => {
  ...
  const handleMenuClick: any = (e: any, id: string) => {
    if (e?.key === "edit") {
      navigate(`/dashboard/claim/${id}/edit`);
  ...
  const columns: TableColumnsType = [
    ...
    { title: 'Statut', dataIndex: 'status', key: 'status' ,render: (text:string) => text === "PENDING" ? <Tag color="#C19A6B">En attente</Tag> :
        text === "IN_PROGRESS" ? <Tag color="black">En cours</Tag>:
          text === "DONE" ? <Tag color="green">Traité</Tag> : <Tag color="Crimson">Rejetée</Tag>
    },
    ...
    <div>
      <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between' }}>
        ...
        <div style={{marginTop:15}} className="d-flex justify-content-end">
          ...
          <Button type="primary" onClick={() => navigate('/dashboard/claim/new')}>Créer une nouvelle réclamation</Button>
        </div>
      </div>
      <Table
