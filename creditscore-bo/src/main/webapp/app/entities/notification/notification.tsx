    ...
    <div>
      ...
      <div className="table-responsive">
        {notificationList && notificationList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                ...
                <th className="hand" onClick={sort('readed')}>
                  Readed <FontAwesomeIcon icon={getSortIconByFieldName('readed')} />
                </th>
            ...
            <tbody>
              {notificationList.map((notification, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    {notification.memberUser ? (
                      <Link to={`/member-user/${notification.memberUser.id}`}>{notification.memberUser.id}</Link>
                    ) : (
                      ''
                    )}
                  </td>
          ...
          !loading && <div className="alert alert-warning">Aucun Notification trouvé</div>
        )}
      </div>
      {totalItems ? (
