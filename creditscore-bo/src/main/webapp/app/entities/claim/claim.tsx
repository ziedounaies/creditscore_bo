    ...
    <div>
      ...
      <div className="table-responsive">
        {claimList && claimList.length > 0 ? (
            ...
            <tbody>
              {claimList.map((claim, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{claim.title}</td>
                  <td>{claim.message}</td>
                  <td>{claim.createdAt ? <TextFormat type="date" value={claim.createdAt} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>{claim.memberUser ? <Link to={`/member-user/${claim.memberUser.id}`}>{claim.memberUser.id}</Link> : ''}</td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        tag={Link}
                        to={`/claim/${claim.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      ...
                      <Button
                        onClick={() =>
                          (window.location.href = `/claim/${claim.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`)
                        }
                        color="danger"
                        size="sm"
          ...
          !loading && <div className="alert alert-warning">Aucun Claim trouvé</div>
        )}
      </div>
      {totalItems ? (
